...
int main(int argc, char *argv[]){
    ...
    char lines[MAXSZ];
    char *p=lines;
    char *x_argv[MAXARG]={0};//参数指针数组全部初始化为空指针

    //存储原有的参数
    for(int i=1;i<argc;i++){ //循环从 1 开始（跳过程序名 argv[0]）
        x_argv[i-1]=argv[i];
    }
    ...
    enum state st = S_WAIT;   // 起始状态置为 S_WAIT
    ...
    while(st!=S_END){
        ...
        if(++arg_end>=MAXSZ){
            fprintf(2,"xagrs: arguments too long.\n");
            exit(1);
        }
        ...
        case S_ARG_LINE_END: //将参数地址存入x_argv数组中同时执行指令
            x_argv[arg_cnt++]=&lines[arg_beg];
            //不加break，后续处理同S_LINE_END
        case S_LINE_END: //行结束，则为当前执行指令
            arg_beg=arg_end;
            *p='\0';
            if(fork()==0){ //fork() 创建子进程，子进程执行 exec
                exec(argv[1],x_argv);
            }
            arg_cnt=argc-1;
